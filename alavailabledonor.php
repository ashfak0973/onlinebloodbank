...
$sql=mysql_query("SELECT `name`, `email`, `bloodgroup`,`lastdonation`,`nextdonation` ,`contact`,`location`, `gender` FROM `beadonor`");
...
{
	$email=$_GET['email'];
	$lastdonation = $_GET['lastdonation'];
	$next = $_GET['nextdonation'];
	$today = date("y-m-d"); 	
	if(($lastdonation+(90 * 24 * 60 * 60))<=$today)
	{
		$next=1;
...
<body  id="page-top">
	...
	<input placeholder="Search by name" name="term" ><input type="submit" value="Submit" name="search" />
	</form></center>
	<br/><br/><br/>
        ...
        <div >
	<center><h3 style="color:white;">Available Donors<font color="white"></h3></center>	<br/><br/><br/>
<style>
table {
...
$sql3 = "SELECT * FROM beadonor WHERE name LIKE '%".$term."%'"; 
$r_query3 = mysql_query($sql3); 
   while($row3=mysql_fetch_array($r_query3))
  {
	  $next=$row3['nextdonation'];
